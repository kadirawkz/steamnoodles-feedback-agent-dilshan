...
# Paths & constants
# ---------------------------
DATA_PATH = r"D:\repos\steamnoodles-feedback-agent-dilshan\data set\Yelp Restaurant Reviews.csv"
...
def get_sentiment_pipe():
    ...
    if _sentiment_pipe is None:
        _sentiment_pipe = pipeline("text-classification", model=_SENTIMENT_MODEL_ID, tokenizer=_SENTIMENT_MODEL_ID, top_k=None)
...
def maybe_paraphrase(reply: str, enable_generation: bool) -> str:
    ...
    if len(paraphrased) > 520:
        paraphrased = paraphrased[:520].rsplit(" ", 1)[0] + "…"
    return paraphrased
